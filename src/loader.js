...
Echo.Loader._initCanvases = function(canvases) {
	...
	Echo.jQuery.map(collection, function(canvas) {
		Echo.Loader._fetchCanvasConfig(canvas.id, function(config) {
			if (!config) return;
...
Echo.Loader._fetchCanvasConfig = function(canvasId, callback) {
	Echo.jQuery.get(Echo.Loader.config.storageURL + canvasId, function(response) {
		if (!response) {
			Echo.Loader._error({
				"args": response,
